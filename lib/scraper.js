...
async function fetchPage(vocationValue, page) {
  try {
    const res = await axios.get(
      `https://thirus2d.online/highscores?type=7&vocation=${vocationValue}&page=${page}`,
      { timeout: 10000 }
    );
    return parseHighscorePage(res.data);
  } catch (err) {
...
export async function fetchHighscores() {
  // Buscar TODAS as páginas de todas as classes em paralelo (25 requests)
  const requests = [];
  for (const voc of VOCATIONS) {
    for (let page = 1; page <= MAX_PAGES; page++) {
      requests.push(fetchPage(voc.value, page));
    }
  }
